...
def test_ica_enabled(delta, tmp_path):
...
def test_minimal_gas_price(delta):
    w3 = delta.w3
...
def test_native_call(delta):
    """
    test contract native call on delta network
...
def test_statesync(delta):
    ...
    # coins = "10basetcro"
    # node = delta.node_rpc(0)
    # txhash_0 = delta.cosmos_cli(0).transfer(from_addr, to_addr, coins)["txhash"]
    ...
    # We can not use the delta fixture to do statesync, since they are full nodes.
    # We can only create a new node with statesync config
    data = Path(delta.base_dir).parent  # Same data dir as delta fixture
    chain_id = delta.config["chain_id"]  # Same chain_id as delta fixture
    cmd = "deltad"
...
def test_local_statesync(delta, tmp_path_factory):
    """
    ...
    delta.supervisorctl("stop", "delta_777-1-node0")
    ...
    delta.supervisorctl("start", "delta_777-1-node0")
    wait_for_port(ports.evmrpc_port(delta.base_port(0)))

    home = tmp_path_factory.mktemp("local_statesync")
    print("home", home)

    i = len(delta.config["validators"])
    ...
    peers = ",".join(
        [
            "tcp://%s@%s:%d"
            % (
                delta.cosmos_cli(i).node_id(),
                val["hostname"],
                ports.p2p_port(val["base_port"]),
            )
            for i, val in enumerate(delta.config["validators"])
        ]
    )
    rpc_servers = ",".join(delta.node_rpc(i) for i in range(2))
    ...
    with subprocess.Popen(
        [delta.chain_binary, "start", "--home", home],
...
def test_failed_transfer_tx(delta):
    """
    ...
    # test the delta_getTransactionReceiptsByBlock api
...
def test_tx_inclusion(delta, max_gas_wanted):
    """
    ...
    modify_command_in_supervisor_config(
        delta.base_dir / "tasks.ini",
        lambda cmd: fn(cmd),
    )
    delta.supervisorctl("update")
    wait_for_port(ports.evmrpc_port(delta.base_port(0)))
...
def test_replay_protection(delta):
    w3 = delta.w3
...
@pytest.mark.gov
def test_submit_any_proposal(delta, tmp_path):
    submit_any_proposal(delta, tmp_path)


@pytest.mark.gov
def test_submit_send_enabled(delta, tmp_path):
...
def test_block_stm_delete(delta):
    """
