...
def test_versiondb_migration(delta: Delta):
    """
    ...
    print(delta.supervisorctl("stop", "all"))
    cli0 = delta.cosmos_cli(i=0)
    cli1 = delta.cosmos_cli(i=1)

    changeset_dir = tempfile.mkdtemp(dir=delta.base_dir)
    ...
    snapshot_dir = tempfile.mkdtemp(dir=delta.base_dir)
    ...
    sst_dir = tempfile.mkdtemp(dir=delta.base_dir)
    ...
    for i in range(len(delta.config["validators"])):
        wait_for_port(ports.evmrpc_port(delta.base_port(i)))
