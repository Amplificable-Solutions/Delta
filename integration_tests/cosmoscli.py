...
from .utils import DELTA_ADDRESS_PREFIX, get_sync_info
...
def module_address(name, prefix=DELTA_ADDRESS_PREFIX):
    """
...
class CosmosCLI:
    ...
    def migrate_delta_genesis(self, version, path):
        return json.loads(
            self.raw(
                "tx",
                "delta",
    ...
    def set_delegate_keys(self, val_addr, acc_addr, eth_addr, signature, **kwargs):
        """
        val_addr: delta validator address
        acc_addr: orchestrator's delta address
    ...
    def query_params(self, module="delta", **kwargs):
        res = json.loads(
            self.raw(
                "query",
                module,
