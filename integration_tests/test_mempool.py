...
def delta_mempool(tmp_path_factory):
    path = tmp_path_factory.mktemp("delta-mempool")
...
def test_mempool(delta_mempool):
    w3: Web3 = delta_mempool.w3
    filter = w3.eth.filter("pending")
    assert filter.get_new_entries() == []

    cli = delta_mempool.cosmos_cli(0)
    # test contract
...
def test_blocked_address(delta_mempool):
    cli = delta_mempool.cosmos_cli(0)
