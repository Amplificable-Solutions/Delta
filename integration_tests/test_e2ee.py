...
def gen_validator_identity(delta: Delta):
    for i in range(len(delta.config["validators"])):
        cli = delta.cosmos_cli(i)
        ...
        delta.supervisorctl("restart", f"delta_777-1-node{i}")

    wait_for_new_blocks(delta.cosmos_cli(), 1)


def test_encrypt_decrypt(delta):
    gen_validator_identity(delta)

    cli0 = delta.cosmos_cli()
    cli1 = delta.cosmos_cli(1)
...
def get_nonce(cli, user):
    acc = cli.query_account(user)["account"]["value"]
    return int(acc.get("sequence", 0))


def test_block_list(delta):
    gen_validator_identity(delta)
    cli = delta.cosmos_cli()
...
def test_invalid_block_list(delta):
    cli = delta.cosmos_cli()
    cipherfile = cli.data_dir / "ciphertext"
