...
from .network import setup_delta, setup_geth
from .utils import (
    ACCOUNTS,
    ADDRS,
    CONTRACTS,
...
def check_auto_deployment(cli, denom, delta_w3, recipient, amount):
    ...
    crc21_contract = delta_w3.eth.contract(
        address=rsp["auto_contract"], abi=delta_crc21_abi()
    )
    try:
...
def get_id_from_receipt(receipt):
    ...
    target = HexBytes(
        abi.event_signature_to_log_topic("__DeltaSendToEvmChainResponse(uint256)")
    )
...
def custom_delta(request, tmp_path_factory):
    yield from setup_delta(tmp_path_factory.mktemp("delta"), 26600, request.param)


@pytest.fixture(scope="module")
def gravity(custom_delta, custom_geth):
    yield from prepare_gravity(custom_delta, custom_geth)


def test_gravity_transfer(gravity):
    geth = gravity.geth
    cli = gravity.delta.cosmos_cli()
    delta_w3 = gravity.delta.w3
    ...
    if gravity.delta.enable_auto_deployment:
        ...
        txreceipt = send_transaction(delta_w3, tx, KEYS["community"])
        # CRC20 emit 3 logs for send_to_evm_chain:
        # burn
        # __DeltaSendToEvmChain
        # __DeltaSendToEvmChainResponse
...
def test_multiple_attestation_processing(gravity):
    if not gravity.delta.enable_auto_deployment:
        geth = gravity.geth
        cli = gravity.delta.cosmos_cli()
        ...
        "messages": [
            {
                ...
                "content": {
                    "@type": "/delta.TokenMappingChangeProposal",
                    "denom": denom,
                    "contract": conctract,
                    "symbol": "",
                    "decimal": 0,
...
def test_gov_token_mapping(gravity, tmp_path, is_legacy):
    ...
    - deploy corresponding contract on delta
    - add the token mapping on delta using gov module
    ...
    approve_proposal(gravity.delta, rsp["events"])
    ...
    print("try to send token from ethereum to delta")
    txreceipt = send_to_cosmos(
    ...
    wait_for_fn("check balance on delta", check)

    # check duplicate end_block_events
    height = cli.block_height()
    port = ports.rpc_port(gravity.delta.base_port(0))
...
def test_gravity_cancel_transfer(gravity):
    if gravity.delta.enable_auto_deployment:
        ...
        cancel_contract = deploy_contract(
            delta_w3,
            CONTRACTS["DeltaGravityCancellation"],
        )
        ...
        def local_check_auto_deployment():
            nonlocal crc21_contract
            crc21_contract = check_auto_deployment(
                cli, denom, delta_w3, community, amount
            )
            return crc21_contract
        ...
        current_block = delta_w3.eth.get_block_number()
        ...
        canceltxreceipt = send_transaction(delta_w3, canceltx, KEYS["community"])
...
def test_gravity_source_tokens(gravity):
    if not gravity.delta.enable_auto_deployment:
        ...
        w3 = gravity.delta.w3
        symbol = "DOG"
        contract, denom = setup_token_mapping(gravity.delta, "TestERC21Source", symbol)
        ...
        # Send back token to delta
        delta_receiver = "0x0000000000000000000000000000000000000001"
        balance_before_send_to_cosmos = contract.caller.balanceOf(delta_receiver)
        amount = 15
        txreceipt = send_to_cosmos(
            gravity.contract,
            cosmos_erc20_contract,
            gravity.geth,
            HexBytes(delta_receiver),
            amount,
            KEYS["validator"],
        )
        assert txreceipt.status == 1, "should success"
        ...
        def check_delta_balance_change():
            nonlocal balance_after_send_to_cosmos
            balance_after_send_to_cosmos = contract.caller.balanceOf(delta_receiver)
            return balance_before_send_to_cosmos != balance_after_send_to_cosmos

        wait_for_fn("check delta balance change", check_delta_balance_change)
