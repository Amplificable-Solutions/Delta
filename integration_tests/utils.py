...
TEST_CONTRACTS = {
    ...
    "DeltaGravityCancellation": "DeltaGravityCancellation.sol",
...
CONTRACTS = {
    ...
    / "x/delta/types/contracts/ModuleCRC20.json",
    "ModuleCRC21": Path(__file__).parent.parent
    / "x/delta/types/contracts/ModuleCRC21.json",
    "ModuleCRC20Proxy": Path(__file__).parent.parent
    / "x/delta/types/contracts/ModuleCRC20Proxy.json",
    **{
...
def submit_gov_proposal(delta, tmp_path, **kwargs):
    ...
    rsp = delta.cosmos_cli().submit_gov_proposal(proposal, from_="community")
    assert rsp["code"] == 0, rsp["raw_log"]
    approve_proposal(delta, rsp["events"])
...
def eth_to_bech32(addr, prefix=DELTA_ADDRESS_PREFIX):
...
def delta_address_from_mnemonics(mnemonics, prefix=DELTA_ADDRESS_PREFIX):
    "return delta address from mnemonics"
...
def send_to_cosmos(gravity_contract, token_contract, w3, recipient, amount, key=None):
    """
    do approve and sendToDelta on ethereum side
    """
    acct = Account.from_key(key)
    ...
    return send_transaction(
        w3,
        gravity_contract.functions.sendToDelta(
...
def get_receipts_by_block(w3, blk):
    if isinstance(blk, int):
        blk = hex(blk)
    rsp = w3.provider.make_request("delta_getTransactionReceiptsByBlock", [blk])
    if "error" not in rsp:
        rsp["result"] = [
...
def multiple_send_to_cosmos(gcontract, tcontract, w3, recipient, amount, keys):
    ...
    for key_from in keys:
        ...
        # generate the tx
        tx = gcontract.functions.sendToDelta(
...
def setup_token_mapping(delta, name, symbol):
    ...
    rsp = delta_cli.query_denom_by_contract(contract.address)
...
def submit_any_proposal(delta, tmp_path):
