...
from .ibc_utils import (
    RATIO,
    assert_duplicate,
    delta_transfer_source_tokens,
    delta_transfer_source_tokens_with_proxy,
    ibc_denom,
...
def test_ibc(ibc):
    # chainmain-1 relayer -> delta_777-1 signer2
    w3 = ibc.delta.w3
    wait_for_new_blocks(ibc.delta.cosmos_cli(), 1)
    ...
    delta_addr = module_address("delta")
    transfer_addr = module_address("transfer")
    seq = get_send_packet_seq(chainmain_cli)
    expected = [
        recv_packet(seq, relayer0, delta_signer2, src_amount, src_denom),
        denom_trace(denom),
        *send_from_module_to_acc(transfer_addr, delta_signer2, src_amount, denom),
        fungible(delta_signer2, relayer, src_amount, src_denom),
        *send_from_acc_to_module(delta_signer2, delta_addr, src_amount, denom),
        *send_from_module_to_acc(delta_addr, delta_signer2, dst_amount, dst_denom),
        write_ack(seq, relayer0, delta_signer2, src_amount, src_denom),
    ]
    assert len(logs) == len(expected)
    height = logs[0]["blockNumber"]
    assert_duplicate(ibc.delta.base_port(0), height)
...
def test_ibc_incentivized_transfer(ibc):
    ...
    expected = [
        ...
        *send_coins(feeibc_addr, src_relayer, ack_fee, fee_denom),
        distribute_fee(delta_signer2, ""),
        *send_coins(feeibc_addr, delta_signer2, 0, fee_denom),
        fungible(checksum_dst_adr, delta_signer2, amount, dst_denom),
        recv_packet(seq1, dst_adr, delta_signer2, amount, transfer_denom),
        *send_coins(escrow, delta_signer2, amount, dst_denom),
        fungible(delta_signer2, checksum_dst_adr, amount, transfer_denom),
        write_ack(seq1, dst_adr, delta_signer2, amount, transfer_denom),
    ]
    assert len(logs) == len(expected)
    for i, log in enumerate(logs):
        method_name, topic = get_topic_data(w3, method_map, contract_info, log)
...
def assert_transfer_source_tokens_topics(ibc, fn):
    ...
    seq0 = get_send_packet_seq(
        ibc.delta.cosmos_cli(),
        criteria="message.action='/ethermint.evm.v1.MsgEthereumTx'",
    )
    seq1 = get_send_packet_seq(ibc.chainmain.cosmos_cli())
    checksum_dst_adr = to_checksum_address(bech32_to_eth(dst_adr))
    delta_addr = module_address("delta")
    delta_denom = f"delta{contract}"
    transfer_denom = f"transfer/{channel}/{delta_denom}"
    expected = [
        acknowledge_packet(seq0),
        fungible(checksum_dst_adr, ADDRS["validator"], amount, delta_denom),
        recv_packet(seq1, dst_adr, delta_signer2, amount, transfer_denom),
        *send_coins(escrow, delta_signer2, amount, delta_denom),
        fungible(delta_signer2, checksum_dst_adr, amount, transfer_denom),
        *send_coins(delta_signer2, delta_addr, amount, delta_denom),
        coin_spent(delta_addr, amount, delta_denom),
        burn(delta_addr, amount, delta_denom),
        write_ack(seq1, dst_adr, delta_signer2, amount, transfer_denom),
    ]
    assert len(logs) == len(expected)
    height = logs[0]["blockNumber"]
...
def test_delta_transfer_source_tokens(ibc):
    assert_transfer_source_tokens_topics(ibc, delta_transfer_source_tokens)


def test_delta_transfer_source_tokens_with_proxy(ibc):
    assert_transfer_source_tokens_topics(ibc, delta_transfer_source_tokens_with_proxy)
