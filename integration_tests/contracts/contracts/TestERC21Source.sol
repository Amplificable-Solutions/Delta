...
contract TestERC21Source is ERC20 {
	...
	event __DeltaSendToIbc(address indexed sender, uint256 indexed channel_id, string recipient, uint256 amount, bytes extraData);
	event __DeltaSendToEvmChain(address indexed sender, address indexed recipient, uint256 indexed chain_id, uint256 amount, uint256 bridge_fee, bytes extraData);
	event __DeltaCancelSendToEvmChain(address indexed sender, uint256 id);
    ...
    **/
	...
	/**
        Internal functions to be called by delta module
    **/
	function mint_by_delta_module(address addr, uint amount) public {
		require(msg.sender == module_address);
		_mint(addr, amount);
	}

	function burn_by_delta_module(address addr, uint amount) public {
		require(msg.sender == module_address);
		unsafe_burn(addr, amount);
	}

	function transfer_by_delta_module(address addr, uint amount) public {
		require(msg.sender == module_address);
		unsafe_transfer(addr, module_address, amount);
	}

	function transfer_from_delta_module(address addr, uint amount) public {
		require(msg.sender == module_address);
    ...
    **/
	...
	function send_to_ibc(string memory recipient, uint amount) public {
		...
		emit __DeltaSendToIbc(msg.sender, recipient, amount);
	}
	...
	function send_to_ibc_v2(string memory recipient, uint amount, uint channel_id, bytes memory extraData) public {
		...
		emit __DeltaSendToIbc(msg.sender, channel_id, recipient, amount, extraData);
	}
	...
	function send_to_evm_chain(address recipient, uint amount, uint chain_id, uint bridge_fee, bytes calldata extraData) external {
		...
		emit __DeltaSendToEvmChain(msg.sender, recipient, chain_id, amount, bridge_fee, extraData);
	}
	...
	function cancel_send_to_evm_chain(uint256 id) external {
		emit __DeltaCancelSendToEvmChain(msg.sender, id);
	}
