...
def prepare_gravity(custom_delta, custom_geth):
    """
    - set-delegator-keys
    - deploy gravity contract
    - start orchestrator
    """
    chain_id = "delta_777-1"
    w3 = custom_geth.w3
    # set-delegate-keys
    for i, val in enumerate(custom_delta.config["validators"]):
        # generate gorc config file
        gorc_config_path = custom_delta.base_dir / f"node{i}/gorc.toml"
        grpc_port = ports.grpc_port(val["base_port"])
        metrics_port = 3000 + i
        gorc_config_path.write_text(
            dump_toml(
                gorc_config(
                    custom_delta.base_dir / f"node{i}/orchestrator_keystore",
        ...
        gorc.add_eth_key("delta")  # delta and eth key derivation are the same
        ...
        acc_addr = gorc.show_cosmos_addr("delta")
        print("fund 100cro to address", acc_addr)
        rsp = custom_delta.cosmos_cli().transfer(
            "community", acc_addr, "%dbasetcro" % (100 * (10**18))
        )
        assert rsp["code"] == 0, rsp["raw_log"]

        cli = custom_delta.cosmos_cli(i)
    ...
    for i, val in enumerate(custom_delta.config["validators"]):
        ...
        programs[f"program:{chain_id}-orchestrator{i}"] = {
            "command": (
                ...
                "--cosmos-key delta --ethereum-key eth --mode AlwaysRelay"
            ),
    ...
    add_ini_sections(custom_delta.base_dir / "tasks.ini", programs)
    custom_delta.supervisorctl("update")

    yield GravityBridge(custom_delta, w3, contract)
...
def setup_cosmos_erc20_contract(cluster, denom, symbol):
    ...
    delta_cli = cluster.delta.cosmos_cli()
    wait_for_new_blocks(delta_cli, 30)
    # Check mapping is done on cluster side
    cosmos_erc20 = delta_cli.query_gravity_contract_by_denom(denom)
