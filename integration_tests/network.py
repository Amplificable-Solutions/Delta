...
class Delta:
    def __init__(self, base_dir, chain_binary="deltad"):
        self._w3 = None
        ...
        )["app_state"]["delta"]["params"]["enable_auto_deployment"]
        self._use_websockets = False
        self.chain_binary = chain_binary

    def copy(self):
        return Delta(self.base_dir)
...
class GravityBridge:
    ...
    def __init__(self, delta, geth, contract):
        self.delta = delta
        self.geth = geth
        self.contract = contract


def setup_custom_delta(
    path,
    base_port,
    config,
    post_init=None,
    ...
    try:
        ...
        c = Delta(path / "delta_777-1", chain_binary=chain_binary or "deltad")
