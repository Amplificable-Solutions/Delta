...
from .network import Delta, setup_custom_delta
from .utils import (
    ADDRS,
    CONTRACTS,
    approve_proposal,
...
def custom_delta(tmp_path_factory):
    yield from setup_delta_test(tmp_path_factory)
...
def setup_delta_test(tmp_path_factory):
    ...
    with contextmanager(setup_custom_delta)(
        ...
        post_init=post_init,
        chain_binary=str(upgrades / "genesis/bin/deltad"),
    ) as delta:
        yield delta
...
def exec(c, tmp_path_factory):
    ...
    c.supervisorctl(
        "start", "delta_777-1-node0", "delta_777-1-node1", "delta_777-1-node2"
    )
    ...
    def do_upgrade(plan_name, target, mode=None):
        ...
        c.chain_binary = (
            Path(c.chain_binary).parent.parent.parent / f"{plan_name}/bin/deltad"
        )
    ...
    c.supervisorctl("stop", "delta_777-1-node0")
    time.sleep(3)
    ...
    c.supervisorctl("start", "delta_777-1-node0")
    wait_for_port(ports.evmrpc_port(c.base_port(0)))
...
def test_cosmovisor_upgrade(custom_delta: Delta, tmp_path_factory):
    exec(custom_delta, tmp_path_factory)
