...
def test_gravity_proxy_contract(gravity):
    if not gravity.delta.enable_auto_deployment:
        ...
        delta_cli = gravity.delta.cosmos_cli()

        print("check the contract mapping not exists yet")
        with pytest.raises(AssertionError):
            delta_cli.query_contract_by_denom(denom)
        ...
        rsp = delta_cli.query_denom_by_contract(crc20.address)
...
def test_gravity_proxy_contract_source_token(gravity):
    if not gravity.delta.enable_auto_deployment:
        ...
        w3 = gravity.delta.w3
        symbol = "TEST"
        contract, denom = setup_token_mapping(gravity.delta, "TestCRC20", symbol)
        ...
        delta_receiver = ADDRS["community"]
...
def test_gravity_non_cosmos_denom(gravity):
    ...
    def check_gravity_native_tokens():
        "check the balance of gravity native token"
        return delta_cli.balance(eth_to_bech32(recipient), denom=denom) == amount
    ...
    # Wait enough for orchestrator to relay the event
    wait_for_new_blocks(delta_cli, 30)

    # Send again token to delta and verify that the network is not stopped
    print("send to delta crc20")
    recipient = HexBytes(ADDRS["community"])
    balance = erc20.caller.balanceOf(ADDRS["validator"])
    txreceipt = send_to_cosmos(
    ...
    def check_gravity_native_tokens():
        "check the balance of gravity native token"
        return delta_cli.balance(eth_to_bech32(recipient), denom=denom) == 2 * amount
