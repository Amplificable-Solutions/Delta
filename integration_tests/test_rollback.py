...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:delta_777-1-node{i}":
            ini[section].update(
                {
...
def custom_delta(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-deltad.nix",
    ]
    print(*cmd)
    broken_binary = Path(subprocess.check_output(cmd).strip().decode()) / "bin/deltad"
    print(broken_binary)
...
def test_rollback(custom_delta):
    """
    ...
    """
    nodes = [3, 4]
    clis = {i: custom_delta.cosmos_cli(i) for i in nodes}
    for i, cli in clis.items():
        wait_for_port(ports.rpc_port(custom_delta.base_port(i)))
    ...
    for i, cli in clis.items():
        ...
        print(f"stop node {i}")
        supervisorctl(
            custom_delta.base_dir / "../tasks.ini", "stop", f"delta_777-1-node{i}"
        )
        ...
        update_node_cmd(custom_delta.base_dir, "deltad", i)

    supervisorctl(custom_delta.base_dir / "../tasks.ini", "update")
