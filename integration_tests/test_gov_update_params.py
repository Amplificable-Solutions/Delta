...
def test_evm_update_param(delta, tmp_path):
    ...
    with pytest.raises(ValueError) as e_info:
        deploy_contract(delta.w3, CONTRACTS["Greeter"])
    assert invalid_msg in str(e_info.value)


def test_gov_update_params(delta, tmp_path):
    params = {
        "delta_admin": "crc12luku6uxehhak02py4rcz65zu0swh7wjsrw0pp",
    ...
    submit_gov_proposal(
        ...
        messages=[
            {
                "@type": "/delta.MsgUpdateParams",
                "authority": authority,
                "params": params,
            }
        ],
    )
    assert delta.cosmos_cli().query_params() == params
