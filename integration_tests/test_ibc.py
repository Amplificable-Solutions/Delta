...
def test_ibc_incentivized_transfer(ibc, tmp_path):
    if not ibc.incentivized:
        ...
        src_chain = ibc.delta.cosmos_cli()
...
def test_delta_transfer_tokens_acknowledgement_error(ibc):
    """
    test sending basetcro from delta to crypto-org-chain using cli transfer_tokens
...
def test_cro_bridge_contract(ibc):
    """
    test sending basetcro from delta to crypto-org-chain using CroBridge contract.
    depends on `test_ibc` to send the original coins.
    """
    dst_addr = ibc.chainmain.cosmos_cli().address("signer2")
    dst_amount = 2
...
def test_delta_transfer_source_tokens(ibc):
    """
    test sending crc20 tokens originated from delta to crypto-org-chain
    """
    assert_ready(ibc)
    delta_transfer_source_tokens(ibc)
...
def test_delta_transfer_source_tokens_with_proxy(ibc):
    ...
    """
    assert_ready(ibc)
    delta_transfer_source_tokens_with_proxy(ibc)
