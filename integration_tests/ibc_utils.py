...
from .network import Chainmain, Delta, Hermes, setup_custom_delta
from .utils import (
    ADDRS,
    CONTRACTS,
    bech32_to_eth,
...
def prepare_network(
    ...
    with contextmanager(setup_custom_delta)(
        tmp_path,
        26700,
        Path(__file__).parent / file,
        relayer=relayer,
    ) as delta:
        cli = delta.cosmos_cli()
        if grantee:
        ...
        chainmain = Chainmain(delta.base_dir.parent / "chainmain-1")
        # wait for grpc ready
        wait_for_port(ports.grpc_port(chainmain.base_port(0)))  # chainmain grpc
        wait_for_port(ports.grpc_port(delta.base_port(0)))  # delta grpc
        wait_for_new_blocks(chainmain.cosmos_cli(), 1)
        wait_for_new_blocks(cli, 1)

        version = {"fee_version": "ics29-1", "app_version": "ics20-1"}
        path = delta.base_dir.parent / "relayer"
        w3 = delta.w3
        contract = None
        acc = None
        ...
        if incentivized:
            register_fee_payee(delta, chainmain, contract, acc)

        port = None
        if is_relay:
            delta.supervisorctl("start", "relayer-demo")
            if is_hermes:
                port = hermes.port
            else:
                port = 5183
        yield IBCNetwork(delta, chainmain, hermes, incentivized)
        if port:
            wait_for_port(port)
...
def ibc_transfer(ibc, transfer_fn=hermes_transfer):
    ...
    dst_denom = "basetcro"
    dst_addr = eth_to_bech32(ADDRS["signer2"])
    old_dst_balance = get_balance(ibc.delta, dst_addr, dst_denom)

    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.delta, dst_addr, dst_denom)
...
def ibc_multi_transfer(ibc):
    chains = [ibc.delta.cosmos_cli(), ibc.chainmain.cosmos_cli()]
...
def ibc_incentivized_transfer(ibc):
    ...
    user0_balances = get_balances(ibc.delta, user0)
    expected = [
...
def delta_transfer_source_tokens(ibc):
    # deploy crc21 contract
    w3 = ibc.delta.w3
    contract, denom = setup_token_mapping(ibc.delta, "TestERC21Source", "DOG")
    ...
    # send back the token to delta
    # check receiver balance
    delta_balance_before_send = contract.caller.balanceOf(ADDRS["signer2"])
    assert delta_balance_before_send == 0

    # send back token through ibc
    print("Send back token through ibc")
    chainmain_cli = ibc.chainmain.cosmos_cli()
    delta_receiver = eth_to_bech32(ADDRS["signer2"])

    coin = "1000" + dest_denom
    fees = "100000000basecro"
    rsp = chainmain_cli.ibc_transfer(
        chainmain_receiver, delta_receiver, coin, "channel-0", fees=fees
    )
    assert rsp["code"] == 0, rsp["raw_log"]

    # check contract balance
    delta_balance_after_send = 0

    def check_contract_balance_change():
        nonlocal delta_balance_after_send
        delta_balance_after_send = contract.caller.balanceOf(ADDRS["signer2"])
        return delta_balance_after_send != delta_balance_before_send

    wait_for_fn("check contract balance change", check_contract_balance_change)
    assert delta_balance_after_send == amount
    return amount, contract.address


def delta_transfer_source_tokens_with_proxy(ibc):
    w3 = ibc.delta.w3
    symbol = "TEST"
    contract, denom = setup_token_mapping(ibc.delta, "TestCRC20", symbol)
    ...
    delta_cli = ibc.delta.cosmos_cli()
    # change token mapping
    rsp = delta_cli.update_token_mapping(
        denom, proxycrc20.address, symbol, 6, from_="validator"
    )
    assert rsp["code"] == 0, rsp["raw_log"]
    wait_for_new_blocks(delta_cli, 1)

    print("check the contract mapping exists now")
    rsp = delta_cli.query_denom_by_contract(proxycrc20.address)
