...
def test_exported_contract(custom_delta):
...
def test_exported_token_mapping(custom_delta):
    cli = custom_delta.cosmos_cli(0)
    port = ports.api_port(custom_delta.base_port(0))
    for case in [
        {
        ...
        url = f"http://127.0.0.1:{port}/delta/v1/contract_by_denom?denom={denom}"
