...
# use deltad on host machine
LOCAL_DELTAD_PATH = "deltad"
DEFAULT_CHAIN_ID = "delta_777-1"
...
def _gen(
    ...
    outdir = Path(outdir)
    cli = ChainCommand(LOCAL_DELTAD_PATH)
...
@click.option(
    "--fromimage", default="ghcr.io/crypto-org-chain/delta-testground:latest"
)
...
@click.option("--deltad", default=CONTAINER_DELTAD_PATH)
@click.option("--global-seq", default=None, type=int)
def run(outdir: str, datadir: str, deltad, global_seq):
    ...
    try:
        return do_run(datadir, home, deltad, group, global_seq, cfg)
    finally:
        # collect outputs
...
def do_run(
    datadir: Path, home: Path, deltad: str, group: str, global_seq: int, cfg: dict
):
    ...
    proc = subprocess.Popen(
        [deltad, "start", "--home", str(home), "--async-check-tx"],
        stdout=logfile,
    )

    cli = ChainCommand(deltad)
    wait_for_port(26657)
    wait_for_port(8545)
