...
You can test with the prebuilt images in [github registry](https://github.com/delta-chain/delta/pkgs/container/delta-testground), or build the image locally:
...
$ docker load < ./result
Loaded image: delta-testground:<imageID>
$ docker tag delta-testground:<imageID> ghcr.io/crypto-org-chain/delta-testground:latest
```
...
docker load < $(nix build .#legacyPackages.aarch64-linux.testground-image --no-link --print-out-paths) \
  | grep "^Loaded image:" \
  | cut -d ' ' -f 3 \
  | xargs -I{} docker tag {} ghcr.io/crypto-org-chain/delta-testground:latest
```

## Generate data files locally

You need to have the `deltad` in `PATH`.
...
```bash
$ nix run github:crypto-org-chain/delta#stateless-testcase patchimage delta-testground:latest /tmp/data/out
```
...
Please use [delta-testground](https://github.com/delta-chain/delta-testground) to run the benchmark in k8s cluster.
