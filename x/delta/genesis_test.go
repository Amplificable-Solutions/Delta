...
func (suite *DeltaTestSuite) TestInitGenesis() {
	testCases := []struct {
		name     string
		malleate func()
		...
		suite.Run(tc.name, func() {
			...
			if tc.expPanic {
				suite.Require().Panics(
					func() {
						delta.InitGenesis(suite.ctx, suite.app.DeltaKeeper, *tc.genState)
					},
				)
			} else {
...
func (suite *DeltaTestSuite) TestExportGenesis() {
	genesisState := delta.ExportGenesis(suite.ctx, suite.app.DeltaKeeper)
