...
var (
	...
	// KeyDeltaAdmin is store's key for the admin address
	KeyDeltaAdmin = []byte("DeltaAdmin")
...
// NewParams creates a new parameter configuration for the delta module
func NewParams(ibcCroDenom string, ibcTimeout uint64, deltaAdmin string, enableAutoDeployment bool, maxCallbackGas uint64) Params {
	return Params{
		IbcCroDenom:          ibcCroDenom,
		IbcTimeout:           ibcTimeout,
		DeltaAdmin:           deltaAdmin,
		EnableAutoDeployment: enableAutoDeployment,
		MaxCallbackGas:       maxCallbackGas,
	}
}

// DefaultParams is the default parameter configuration for the delta module
...
// Validate all delta module parameters
func (p Params) Validate() error {
	...
	if len(p.DeltaAdmin) > 0 {
		if _, err := sdk.AccAddressFromBech32(p.DeltaAdmin); err != nil {
			return err
		}
	}
...
func (p *Params) ParamSetPairs() paramtypes.ParamSetPairs {
	return paramtypes.ParamSetPairs{
		...
		paramtypes.NewParamSetPair(KeyDeltaAdmin, &p.DeltaAdmin, validateIsAddress),
