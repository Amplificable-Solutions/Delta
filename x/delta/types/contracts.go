...
const EVMModuleName = "delta-evm"

var (
	//go:embed contracts/ModuleCRC20.json
	deltaCRC20JSON []byte

	// ModuleCRC20Contract is the compiled delta crc20 contract
	ModuleCRC20Contract CompiledContract

	//go:embed contracts/ModuleCRC21.json
	deltaCRC21JSON []byte

	// ModuleCRC21Contract is the compiled delta crc21 contract
...
func init() {
	...
	err := json.Unmarshal(deltaCRC20JSON, &ModuleCRC20Contract)
	if err != nil {
		panic(err)
	}
	...
	err = json.Unmarshal(deltaCRC21JSON, &ModuleCRC21Contract)
	if err != nil {
		panic(err)
	}
