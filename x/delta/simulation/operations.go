...
func SimulateUpdateTokenMapping(ak types.AccountKeeper, bk types.BankKeeper, k *keeper.Keeper) simtypes.Operation {
	return func(
		...
		deltaAdmin := k.GetParams(ctx).DeltaAdmin
		var simAccount simtypes.Account

		if r.Intn(2) > 0 {
			var found bool
			simAccount, found = findDeltaAdmin(accs, deltaAdmin)
			if !found {
		...
		if simAccount.Address.String() != deltaAdmin && errors.Is(err, errorsmod.Wrap(sdkerrors.ErrUnauthorized, "msg sender is not authorized")) {
...
func findDeltaAdmin(accs []simtypes.Account, deltaAdmin string) (simtypes.Account, bool) {
	found := false
	for _, acc := range accs {
		if acc.Address.String() == deltaAdmin {
			found = true
			return acc, found
		}
	}
