...
import (
	...
	"github.com/delta-chain/delta/v2/x/delta/keeper"
...
func (suite *DeltaTestSuite) SetupTest() {
	checkTx := false
...
func (suite *DeltaTestSuite) TestMsgConvertVouchers() {
	testCases := []struct {
		name          string
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			msgSrv := keeper.NewMsgServerImpl(suite.app.DeltaKeeper)
...
func (suite *DeltaTestSuite) TestMsgTransferTokens() {
	testCases := []struct {
		name          string
...
func (suite *DeltaTestSuite) TestUpdateTokenMapping() {
	suite.SetupTest()
	...
	err := suite.app.DeltaKeeper.RegisterOrUpdateTokenMapping(suite.ctx, msg)
	suite.Require().NoError(err)

	contractAddr, found := suite.app.DeltaKeeper.GetContractByDenom(suite.ctx, denom)
