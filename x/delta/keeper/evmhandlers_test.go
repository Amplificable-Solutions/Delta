...
func (suite *KeeperTestSuite) TestSendToAccountHandler() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			handler := evmhandlers.NewSendToAccountHandler(suite.app.BankKeeper, suite.app.DeltaKeeper)
			tc.malleate()
...
func (suite *KeeperTestSuite) TestSendToIbcHandler() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.app.DeltaKeeper.SetExternalContractForDenom(suite.ctx, invalidDenom, contract)
				coin := sdk.NewCoin(invalidDenom, sdkmath.NewInt(100))
				err := suite.MintCoins(sdk.AccAddress(contract.Bytes()), sdk.NewCoins(coin))
				suite.Require().NoError(err)
			...
			func() {},
			errors.New("the native token associated with the contract 0x0000000000000000000000000000000000000001 is neither an ibc voucher or a delta token"),
		},
		{
			"success send to ibc",
			func() {
				suite.app.DeltaKeeper.SetExternalContractForDenom(suite.ctx, validDenom, contract)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToIbcHandler(suite.app.BankKeeper, deltaKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendToIbcV2Handler() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToIbcV2Handler(suite.app.BankKeeper, deltaKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendCroToIbcHandler() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendCroToIbcHandler(suite.app.BankKeeper, deltaKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
