...
const SendCroToIbcEventName = "__DeltaSendCroToIbc"

// SendCroToIbcEvent represent the signature of
// `event __DeltaSendCroToIbc(string recipient, uint256 amount)`
...
// SendCroToIbcHandler handles `__DeltaSendCroToIbc` log
type SendCroToIbcHandler struct {
	bankKeeper  types.BankKeeper
	deltaKeeper deltakeeper.Keeper
}

func NewSendCroToIbcHandler(bankKeeper types.BankKeeper, deltaKeeper deltakeeper.Keeper) *SendCroToIbcHandler {
...
func (h SendCroToIbcHandler) Handle(
	...
	evmDenom := h.deltaKeeper.GetEvmParams(ctx).EvmDenom
	...
	if err = h.deltaKeeper.IbcTransferCoins(ctx, sender.String(), recipient, coins, ""); err != nil {
		return err
	}
	return nil
}
