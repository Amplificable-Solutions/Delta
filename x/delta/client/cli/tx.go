...
func CmdConvertTokens() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Convert ibc vouchers to delta tokens, Note, the'--from' flag is" +
...
func CmdSendToCryptoOrg() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Transfer delta tokens to the origin chain through IBC , Note, the'--from' flag is" +
...
// CmdUpdatePermissions returns a CLI command handler for updating delta permissions
...
// CmdStoreBlockList returns a CLI command handler for updating delta permissions
...
Example:
$ %s migrate v1.0 /path/to/genesis.json --chain-id=delta_777-1 --genesis-time=2019-04-22T17:00:00Z
`, version.AppName),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
