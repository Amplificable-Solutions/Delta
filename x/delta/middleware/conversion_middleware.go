...
func NewIBCConversionModule(app porttypes.IBCModule, ck deltakeeper.Keeper) IBCConversionModule {
...
func (im IBCConversionModule) convertVouchers(ctx sdk.Context, data transferTypes.FungibleTokenPacketData, denom string, isSender bool) error {
	...
	if isSender {
		im.deltakeeper.OnRecvVouchers(ctx, sdk.NewCoins(token), data.Sender)
	} else {
		im.deltakeeper.OnRecvVouchers(ctx, sdk.NewCoins(token), data.Receiver)
	}
	return nil
}

func (im IBCConversionModule) canBeConverted(ctx sdk.Context, denom string) bool {
	params := im.deltakeeper.GetParams(ctx)
	if denom == params.IbcCroDenom {
		return true
	}
	_, found := im.deltakeeper.GetContractByDenom(ctx, denom)
	return found
}
