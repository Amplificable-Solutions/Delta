...
const (
	// DeltaNamespace is the extension RPC namespace of delta module.
	DeltaNamespace = "delta"
...
func init() {
	if err := evmrpc.RegisterAPINamespace(DeltaNamespace, CreateDeltaRPCAPIs); err != nil {
		panic(err)
	}
}

// CreateDeltaRPCAPIs creates extension json-rpc apis
func CreateDeltaRPCAPIs(ctx *server.Context, clientCtx client.Context, _ *stream.RPCStream, allowUnprotectedTxs bool, indexer ethermint.EVMTxIndexer) []rpc.API {
	evmBackend := backend.NewBackend(ctx, ctx.Logger, clientCtx, allowUnprotectedTxs, indexer)
	return []rpc.API{
		{
			Namespace: DeltaNamespace,
			Version:   apiVersion,
			Service:   NewDeltaAPI(ctx.Logger, clientCtx, *evmBackend),
			Public:    true,
		},
	}
}

// DeltaAPI is the extension jsonrpc apis prefixed with delta_.
type DeltaAPI struct {
	...
	deltaQueryClient types.QueryClient
}

// NewDeltaAPI creates an instance of the delta web3 extension apis.
func NewDeltaAPI(
	logger log.Logger,
	clientCtx client.Context,
	backend backend.Backend,
	...
	return &DeltaAPI{
		...
		backend:          backend,
		deltaQueryClient: types.NewQueryClient(clientCtx),
	}
}

func (api *DeltaAPI) getBlockDetail(blockNrOrHash rpctypes.BlockNumberOrHash) (
...
func (api *DeltaAPI) GetTransactionReceiptsByBlock(blockNrOrHash rpctypes.BlockNumberOrHash) ([]map[string]interface{}, error) {
	api.logger.Debug("delta_getTransactionReceiptsByBlock", "blockNrOrHash", blockNrOrHash)
...
func (api *DeltaAPI) ReplayBlock(blockNrOrHash rpctypes.BlockNumberOrHash, postUpgrade bool) ([]map[string]interface{}, error) {
	api.logger.Debug("delta_replayBlock", "blockNrOrHash", blockNrOrHash)
	resBlock, blockNumber, blockHash, blockRes, baseFee, err := api.getBlockDetail(blockNrOrHash)
	if err != nil {
		return nil, err
	}
	...
	rsp, err := api.deltaQueryClient.ReplayBlock(rpctypes.ContextWithHeight(contextHeight), req)
	if err != nil {
		return nil, err
	}
...
func (api *DeltaAPI) getBlock(blockNrOrHash rpctypes.BlockNumberOrHash) (blk *coretypes.ResultBlock, err error) {
