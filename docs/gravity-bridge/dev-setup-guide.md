...
- `deltad`, the delta node binary.
- `gorc`, the gravity bridge orchestrator cli, built from the [crypto-org fork](https://github.com/crypto-org-chain/gravity-bridge/tree/v2.0.0-delta/orchestrator/gorc).
- `pystarport`, a tool to run local cosmos devnet.
- `start-geth`/`start-delta`, convenient scripts to start the local devnets.

Clone delta repo locally and run `nix-shell integration_tests/shell.nix` in it, you'll get a virtual shell with the
...
You can either use a public delta testnet (that have embed the gravity-bridge module), or run `start-delta
/tmp/test-delta` to get a local Delta testnet.
...
You need to prepare two accounts for the orchestrator, one for ethereum and one for delta. You should transfer some funds to these accounts, so the orchestrator can cover the gas fees of message relaying later.
...
grpc = "http://localhost:9090" # TO BE UPDATED - GRPC of Delta node
key_derivation_path = "m/44'/60'/0'/0/0"
prefix = "tcrc"
...
ethereum key, and send it to a delta validator to register it.

The protobuf message is like this:

```protobuf
message DelegateKeysSignMsg {
  // The valoper prefixed delta validator address
...
At last, send the orchestrator's ethereum address, delta address, and the signature we just signed above to a Delta
validator, the validator should send a `set-delegate-keys` transaction to delta network to register the binding:

```shell
$ deltad tx gravity set-delegate-keys $val_address $orchestrator_delta_address $orchestrator_eth_address $signature
```
...
- `gravity_id`. Run command `deltad q gravity params | jq ".params.gravity_id"`
- `threshold`, constant `2834678415`, which is just `int(2 ** 32 * 0.66)`.
- `eth_addresses` and `powers`:
  - Query signer set by running command: `deltad q gravity latest-signer-set-tx | jq ".signer_set.signers"`
...
After all the orchestrator processes run, the gravity bridge between ethereum and delta is setup successfully.
