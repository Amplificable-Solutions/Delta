# ADR 009: Permissioned addresses in Delta
...
Some messages in delta require permissions. For example, changing the mapping to support new CRC20 auto-conversion contracts or disabling the bridge module in case of emergency. Right now, only the admin has the ability to use those messages.
 The admin is a single address defined in delta space and can be modified through governance. It is usually a multi-sig address shared by multiple trusted parties in order to achieve a higher level of security.
...
A special address defined in delta space that can be modified through governance. Usually a multi-sig address.

### Permissioned addresses

An address in Delta that is allowed to perform a type of operation (message).

### Non-permissioned addresses

An address in Delta that carry no permissions.


### Option 1 : Create a permission system within delta module for messages.

For the above problem, we propose the following:

- Keep the admin address in Delta space. The admin has all the permissions and has the ability to change the permission of an address.
- By default, all addresses in Delta are non-permissioned
- Assign to each "restricted" messages in Delta a permission (integer value) and create in Delta module a mapping between addresses and permissions that is stored in memory. For now, there are only two messages that require permission : MsgUpdateTokenMapping and MsgTurnBridge.
...
- Simple implementation self-contained in x/delta module. Ability to be customized in the future if required.
...
### Option 2 : Use a subset of x/group module functionality to restrict some transactions in delta

For the above problem, we propose the following:

- For each specific msg, we define a group and for the group, a group policy
- Use the admin defined in delta space as general admin for the group
...
In Delta module, we need to restrict ``MsgUpdateTokenMapping`` and ``MsgTurnBridge`` msg to only specific "group policy address".
...
- completely remove (rollback) and be replaced by the circuit breaker
  (PR to rollback: https://github.com/delta-chain/delta/pull/795 and https://github.com/crypto-org-chain/gravity-bridge/pull/96)
...
- https://github.com/delta-chain/delta/pull/795
