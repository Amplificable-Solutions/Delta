# ADR 004: Tokens conversion in Delta
...
This ADR documents the "auto" token conversion mechanism implemented in Delta which aim to improve user-experience by allowing tokens to be converted to a single type (erc20) while in the underlying chain, multiple type of tokens coexist.
...
This ADR describes a mechanism to convert seamlessly native tokens to evm tokens (erc20) out of an IBC transfer. In a more general concept, any kind of native tokens can be converted to an evm tokens (and vice versa) using delta native transactions. 
...
- Implement in x/delta module two cosmos transactions : `convert-vouchers` allows to convert an ibc-originated asset to its erc20 representation and `transfer-tokens` allows to send a erc20 to another cosmos chain through ibc.
...
- mint_by_delta_module(address user, uint256 amount)
- burn_by_delta_module(address user, uint256 amount)
...
-  __DeltaSendToAccount(address recipient, uint256 amount) : send a specific token amount from contract address to recipient address
-  __DeltaSendToIBCChannel(address recipient, uint256 amount) : send a specific token amount through ibc to recipient address in destination chain
...
* https://github.com/delta-chain/delta/blob/main/contracts/src/ModuleCRC20.sol
* https://github.com/delta-chain/delta/blob/main/docs/architecture/adr-002.md