...
Right now Delta module supports native token to CRC20 token conversion only. As more dApp projects deploy their tokens on Delta EVM, there is a need to support converting EVM tokens deployed on Delta EVM to native token for use cases such as bridging to Cosmos Chains by IBC or to Ethereum by Gravity Bridge.
...
EVM Tokens are tokens that adopt the widely used CRC20/CRC21 token standard. These token are smart contracts deployed on Delta EVM.

### Local EVM Token

Local EVM Tokens are EVM tokens that are originated from Delta EVM, that means the minting of the tokens happen on Delta.

### Native Token

Native Tokens are tokens on the Cosmos layer of Delta. They are not directly usable on Delta EVM but an intermediate token format between bridge protocols and Delta EVM because most bridge protocols are Cosmos module that interacts with native tokens.
...
Currently, the native token denom to contract address is registered in the Delta module kvstore and stored as key-value pairs: 
```
key -> value
...
For example, IBC-ATOM to contract on Delta mainnet mapping is
```
...
- `delta0x`: Delta CRC20/CRC21 Native Token
...
In this ADR, we do not intend to change the mapping registration process. Following the current design, the address specified in the network parameter `DeltaAdmin` has the rights to update the mappings.
...
2. If the token source is Delta EVM, invoke `keeper.BurnVouchersToEvmCoins()`; Otherwise, invoke `keeper.ConvertVouchersToEvmCoins()`^2;
...
The denom naming of the native token with Delta EVM as source is
```
delta0x{smartContractAddress}
```

The chain must avoid naming any native asset with the prefix `delta0x` in the future.
...
3. Invoke the corresponding smart contract `mint_by_delta_module` method to mint the EVM token to `receiver` EVM address.
