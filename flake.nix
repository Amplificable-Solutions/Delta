{
  ...
  outputs =
    ...
    (flake-utils.lib.eachDefaultSystem (
      ...
      in
      rec {
        packages = pkgs.delta-matrix // {
          inherit (pkgs) rocksdb testground-image;
        };
        apps = {
          deltad = mkApp packages.deltad;
          deltad-testnet = mkApp packages.deltad-testnet;
          stateless-testcase = {
            type = "app";
            program = "${pkgs.benchmark-testcase}/bin/stateless-testcase";
          };
        };
        defaultPackage = packages.deltad;
        defaultApp = apps.deltad;
        devShells = rec {
    ...
    // {
      overlays.default = [
        ...
        (final: super: {
          ...
          delta-matrix = final.callPackage ./nix/delta-matrix.nix {
            inherit rev;
