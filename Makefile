...
ldflags += -X github.com/cosmos/cosmos-sdk/version.Name=delta \
	-X github.com/cosmos/cosmos-sdk/version.AppName=deltad \
...
build: check-network print-ledger go.sum
	@go build -mod=readonly $(BUILD_FLAGS) -o $(BUILDDIR)/deltad ./cmd/deltad

install: check-network print-ledger go.sum
	@go install -mod=readonly $(BUILD_FLAGS) ./cmd/deltad
...
SIMAPP = github.com/delta-chain/delta/v2/app
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.deltad/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.deltad/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
test-delta-contracts:
	@git submodule update --init --recursive
	@nix-shell ./contracts/shell.nix --pure --run ./scripts/test-delta-contracts

gen-delta-contracts:
	@git submodule update --init --recursive
	@nix-shell ./contracts/shell.nix --pure --run ./scripts/gen-delta-contracts

gen-bindings-contracts:
	@nix-shell ./nix/gen-binding-shell.nix --pure --run ./scripts/gen-bindings-contracts

.PHONY: gen-delta-contracts gen-bindings-contracts test-delta-contracts
...
HTTPS_GIT := https://github.com/delta-chain/delta.git
protoVer=0.14.0
