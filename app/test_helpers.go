...
func Setup(t *testing.T, deltaAdmin string) *App {
	t.Helper()
	...
	return SetupWithGenesisValSet(t, deltaAdmin, valSet, []authtypes.GenesisAccount{acc}, balance)
}
...
func SetupWithGenesisValSet(t *testing.T, deltaAdmin string, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *App {
	t.Helper()
	...
	deltaGen := deltatypes.DefaultGenesis()
	deltaGen.Params.DeltaAdmin = deltaAdmin
	// enable auto deployment in test genesis
	deltaGen.Params.EnableAutoDeployment = true
	genesisState["delta"] = app.cdc.MustMarshalJSON(deltaGen)
