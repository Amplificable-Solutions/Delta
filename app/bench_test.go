...
func benchmarkERC20Transfer(b *testing.B, db dbm.DB, appOpts servertypes.AppOptions) {
...

	{
		...
		contractAddr, err = app.DeltaKeeper.DeployModuleCRC21(ctx, "test")
		require.NoError(b, err)
		for _, acc := range testAccounts {
			_, err = app.DeltaKeeper.CallModuleCRC21(ctx, contractAddr, "mint_by_delta_module", acc.Address, big.NewInt(amount))
			require.NoError(b, err)
		}
		write()
	}
	...
	ret, err := app.DeltaKeeper.CallModuleCRC21(ctx, contractAddr, "balanceOf", testAccounts[0].Address)
	require.NoError(b, err)
