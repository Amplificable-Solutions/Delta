    ...
    runCommand "tarball-${drv.name}"
      ...
      ''
        tar cfv - -C "${drv}" \
          ...
          | gzip -9 > $out
      '';
  bundle-win-exe = drv: callPackage ./bundle-win-exe.nix { deltad = drv; };
...
builtins.listToAttrs (
  builtins.map (
    { network, pkgtype }:
    {
      ...
      value =
        let
          deltad = callPackage ../. { inherit rev network; };
          bundle = if stdenv.hostPlatform.isWindows then bundle-win-exe deltad else bundle-exe deltad;
        in
